...
## 4.2.2 依赖维护（2026-09-16）

- 本次 4.2.2 补丁版本包含 Roslyn 4.8 → 5.9、checkout v7、setup-dotnet v6、upload-artifact v7；Dependabot PR #1、#4、#5、#7 已合并，.NET 8、配置结构和脚本接口不变。
- 在隔离工作区审查 Dependabot PR #1、#4、#5、#7，保留原工作区未提交改动。升级组合构建通过；并发 harness 5/5，脚本编译/错误诊断/缓存失效、实时通知语法识别与重置，以及单文件 CLI 的进度/通知预览通过。
- 本机四种现有脚本中三种可编译；另一种缺少 return 和分号，在旧 Roslyn 与新 Roslyn 下均失败，已确认为既有用户脚本问题，未修改实际配置。
- 用户随后明确授权替换本机程序并恢复先前运行的项目：已停止当时的两个启动动作，退出托盘并替换为 4.2.2，源/目标 SHA-256 一致且配置文件未改变；新版托盘唯一，随后仅重新运行原来的两个精确 Action，一次状态回读均为 Running、无 LastError。这是进程状态确认，不代表业务就绪验收。
- 正式 v4.2.2 已发布为 latest，标签指向发布提交 f605073；main 构建通过，Release 仅包含 ServicePilot.exe，资产 SHA-256 与已安装程序一致：`9ef72bd8e3524c65624deac27c6bba9e2bbc4b615dabd806985cd729912cc80f`。未获类似明确停机授权时，仍不得在有活动服务时替换托盘程序。

## 4.2.1 日志全局折叠模式与搜索横向定位（2026-09-02）

- **后续日志继承全局模式**：日志窗口新增窗口级 `_foldNewGroupsByDefault`。工具栏“展开”会展开当前全部合并组，并让之后新出现的组默认展开；“摘要”会折叠当前全部组，并让新组默认折叠。既有 `_foldStateByHeader` 仍保存每组的独立意图，所以单组手动切换或搜索临时展开不会篡改全局模式，清空日志也不会重置本窗口已选择的模式。
- **搜索同时定位 X/Y 轴**：搜索选中命中内容后调用 AvalonEdit `Caret.BringCaretToView()`；目标位于未换行长日志右侧时，横向滚动条会自动移动到选区，不再只定位行号。
- **验证**：临时真实 WPF harness 2/2 通过：全局展开后追加第二组保持展开，切回摘要后追加第三组保持折叠；600 字符长行末尾的命中选区正确，`HorizontalOffset` 从 0 移动到约 3491 像素。临时 harness 已移除，未加入产品文件。
- **验收、构建与本机状态**：用户已确认本轮功能校验成功。最终 Debug 构建为 0 警告、0 错误；4.2.1 Release 单文件发布成功，`dist` 仅含 `ServicePilot.exe`，CLI 版本为 4.2.1，隔离 `doctor --json` 为 0 错误、0 警告，候选包 SHA-256 为 `075adc02e338fe27bf51a0b09a6b90b545cc92192cf1bc4d465a668634785bbb`。覆盖前回读 42 个受管服务均未运行，因此 4.2.0 托盘正常退出后已覆盖为 4.2.1 并隐藏重启；源/目标哈希一致、托盘进程唯一且服务配置数量保持 42。
