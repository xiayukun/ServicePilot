...
- 准备一个完整模板，包含修改配置动作和 `npm run dev` 启动动作。
...
1. 托盘右键主菜单  
   展示数字托盘图标、服务列表、状态小点、启动/执行动作/日志/编辑/模板操作，以及语言切换菜单。

2. 管理服务窗口  
   展示多服务列表、运行状态、动作数量、变量数量，以及启动、执行动作、停止、重启、日志等操作按钮。
...
6. 托盘 AI 帮助窗口
   展示 `复制给 AI 的帮助` 弹窗中的当前 exe 路径、建议先运行命令、多行可复制提示词，以及 `复制全部` / `复制命令` 按钮；截图前不要展示真实隐私路径，必要时使用测试构建目录。
...
- 执行动作子菜单：展示组合动作、单个动作、变量子菜单和运行中动作禁用状态；不要再展示旧的 `启动执行` / `不启动执行` 分组。
