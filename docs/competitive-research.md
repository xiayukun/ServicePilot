...
| [Listr2](https://github.com/listr2/listr2) | 任务列表状态机 | 细粒度任务状态、渲染器、并发和失败控制 | 当前动作状态已够用；未来可加 retry/rollback |
...
- CLI 补齐服务动作变量维护：`step variables`、`step variable-add`、`step variable-remove`、`step variable-clear`。
- CLI 补齐模板动作变量维护：`template step-variables`、`template step-variable-add`、`template step-variable-remove`、`template step-variable-clear`。
