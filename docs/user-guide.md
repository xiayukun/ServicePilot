...
- 运行动作/组合动作、停止服务。
- 按变量运行单个动作或组合动作。
- 按 Tab 查看每个动作自己的输出；页签按稳定动作标识懒创建，动作开始运行时自动切换到对应页签；同名动作不会混入同一页签。
- 单独关闭某个动作页签并清除其已保留日志，或用工具栏“清空”清除当前服务的全部日志；两者都不会停止正在运行的动作。
- 自动滚动会在日志内容和折叠布局完成后定位到底部；摘要/展开按钮会随真实折叠状态同步，并让后续新日志组保持最后选择的全局模式。
- 编辑当前服务。
- 搜索、上一个/下一个匹配；命中长行右侧内容时会同时横向滚动到目标位置。
- 复制选中日志、复制全部日志。
- 横向滚动长行。
