...
- Public release version: `4.2.1`.
- App icon assets are `ServicePilot\Resources\Icons\app.ico` and `app.png`, generated together from the preferred transparent `servicepilot_icon_final.png` via `scripts\make_icon.py`. The multi-size 16-256 ICO backs the exe `<ApplicationIcon>` for Windows shell surfaces. Every FluentWindow `Icon` (taskbar) and each `ui:TitleBar.Icon` (visible left-side title-bar icon) use the PNG directly so WPF does not choose and rescale an ICO frame with a residual light fringe. The generator keeps a full square canvas, makes the four corners transparent, removes the outer white matte with an alpha mask, preserves the previous visible subject size, and uses premultiplied-alpha resizing; the old V1 source is only a fallback. The dynamic tray badge icon is generated separately in `App.CreateTrayIconWithBadge` and intentionally uses neither asset.
...
- Template exchange cloning must preserve each Action's `LogMergeScript`; exporting or importing a template must not drop log folding, progress, or notification behavior.
...
- `ScriptExecutor` guards runner publication, start, stop takeover, clearing, and disposal with `_runnerGate`. The cancellation check immediately before publication/start must remain inside that gate so Stop cannot miss a newly created or about-to-start runner.
...
- `ProcessRunner.Completion` is the single normal-completion boundary: it completes only after the process has exited, both redirected stdout/stderr pumps have reached EOF, and all output already enqueued for subscribers has been delivered. `ScriptExecutor` must await this boundary before publishing action/service completion or disposing the runner, otherwise buffered tail lines can be lost. Stop waits on the same boundary after terminating the Job/process tree.
- `stdout` and `stderr` are pumped by two concurrent tasks. `ProcessRunner.Emit` uses `_emitGate` only to serialize the program-observed enqueue order into a single-reader channel; the channel worker invokes `OutputReceived` and the blocking `Dispatcher.Invoke` strictly outside that lock. This preserves enqueue/delivery order for the order-dependent fold state machine without claiming the absolute OS generation order between stdout and stderr. Never invoke subscribers or a Dispatcher while holding `_emitGate`.
...
- Stop cancels the executor, closes the Windows Job Object, falls back to process-tree kill, and drains process output plus subscriber delivery before publishing one terminal state. A five-second drain timeout triggers another forced kill/stream close, suppresses future delivery, reports `Error`, and propagates `TimeoutException`; it must never be published as a successful `Stopped` result.
...
- The tray menu pins a localized service-name filter above the scrollable service list. Opening the menu clears and focuses the filter so keyboard input works immediately; filtering only changes service-item visibility, while the pinned management footer remains available. Down Arrow moves from the filter to the first visible service, and Escape clears a non-empty query.
...
- Action log tabs are keyed by stable `LogEntry.StepId`, not display names, and expose a close button. Closing an action tab removes that action's existing entries from the application-level log buffer as well as its tab/render/search/fold state; later output may lazily recreate the tab with only new entries. The toolbar `Clear` action is intentionally service-wide and clears the same authoritative buffer so old logs cannot return after reopening the window. Neither operation stops a running action or service.
...
- Log auto-scroll is layout-driven, not timer-driven. A visible-tab render batch requests at most one scroll only after its `TextDocument` insertion, folding rebuild, and redraw; `LogEditor.LayoutUpdated` consumes the request and calls `ScrollToEnd` after AvalonEdit has the latest extent. Turning auto-scroll off cancels any pending request, while turning it on requests an immediate layout-backed scroll. Non-visible tabs must never request scrolling.
...
- Globals contract: `PreviousLine` and `CurrentLine` are the FULL formatted line `"HH:mm:ss [Level] message"` (with timestamp/level prefix). The process-emitted text such as `<s> [webpack.Progress] ...` is part of the message. This is the exact string produced by `LogWindow.FormatLogLine`. Additional globals: `PreviousResult` (`MergeResult?` returned for the previous line — read `PreviousResult.State` to carry state forward), `PreviousWasCollapsed` (`bool`), `InCollapseGroup` (`bool`, whether a group header exists to fold into), and `Notify(string message)`. `Notify` requests a best-effort Windows information notification for a LIVE line; it works with no log window open. Validation, `merge-script test`, and historical replay may collect/preview requests but must never show a system notification. If you add/remove injected globals, update `LogMergeService.BuildSource` locals AND `UserBodyStartLine` together.
...
- Folding is a REAL AvalonEdit fold, not a text overwrite. Raw lines are always kept in the buffer/CLI data; `LogWindow.RebuildFoldings` builds `FoldingManager` sections from the collapse groups, initially folds them, and shows one summary line with a left-side `>`/`+` toggle. Expanding reveals every raw child line. Search (`FindLogMatch`) auto-expands any fold that contains a hit, selects the match (which places AvalonEdit's caret at its end), and calls `Caret.BringCaretToView()` so both vertical and horizontal viewports reveal a match in a long line. The summary button toggles fold-all/expand-all.
- Fold collapse state is remembered per group header in `LogWindow._foldStateByHeader` (keyed by the header `LogEntry`, so it survives section destroy/recreate during incremental rebuilds and tab switches). `RebuildFoldings` captures each live section's `IsFolded` into this dict BEFORE `UpdateFoldings`, verifies that every resulting section exactly matches the newly calculated start/end range, and falls back to a full section rebuild only when that postcondition fails; it then reapplies the remembered intent. Brand-new headers use the window-level `_foldNewGroupsByDefault` mode last chosen by the aggregate Fold/Expand command, initially folded. The summary/expand command reconciles foldings before toggling so a stale trailing group cannot remain outside the command's scope. Do not revert to a "fold once" flag: an actively-streaming group's section can be recreated when new child lines arrive, which previously popped a manually-collapsed group back open (fixed in 4.0.2). Only clear the per-header dict when logs are cleared, not on tab switch; clearing logs does not reset the window-level mode.
- The log toolbar label, tooltip, enabled state, and next action remain derived from the current `FoldingManager.AllFoldings`: any expanded section means `摘要` / fold all; all sections folded means `展开` / expand all. The aggregate command also updates `_foldNewGroupsByDefault`, so future groups follow the chosen mode. Individual fold-margin clicks and search-triggered expansion must capture only the header-keyed intent and refresh the button; they must not change the window-level default. Suppress event capture while `RebuildFoldings` is applying remembered intent.
...
- `LiveLogMergeProcessor` uses Roslyn syntax to recognize real `Notify(...)` invocations (comments and string literals do not count), evaluates only those scripts against every live line at application level, and attaches the transient result to `LogEntry`. `LogWindow` must reuse a matching precomputed result so an open window does not execute the same notification-capable script twice. Clearing an action/service log also clears its live merge state. Existing folding-only scripts remain window-driven, so closed windows do not add background merge overhead.
- One-shot startup notifications must carry their guard through intermediate lines that would otherwise return `null`; for example, a frontend HMR `100%` after the first completion must not notify again. `App.OnProcessStepStateChanged` clears live merge state when an Action enters `Running`, so the guard is re-armed for the next action run.
- The script is read live from the current config on every line, so `merge-script set` / GUI edits / external config edits (via the file watcher, which calls `ProcessManager.UpdateService` → `RuntimeState.Config`) take effect on the next log line without restarting the service. When the script text changes, both the application-level live evaluator and the log tab reset `PreviousLine` / `PreviousResult` / open-group state before evaluating the next line; never carry state across different script texts.
...
- Runtime/system failure logs trigger a throttled tray balloon from `App.OnServiceOutput`. Scripted `Notify` requests use the service name as title, sanitize/truncate the custom body through the same safe balloon path, and de-duplicate identical text from the same service/action for five seconds. Do not notify on every stderr line; notify on final/system failures so retry noise does not spam the user.
...
`SERVICE`, `STEP`, and `TEMPLATE` can be names or GUIDs. `STEP` can also be numeric: `1..N` selects the startup-step display number, while `0` remains a legacy internal-order escape hatch. Script types are `Batch`, `PowerShell`, `Python`, and `Node`. CLI step specs can be `Name|Type|command`, `Name|Type|UseVariable|command`, `Name|Type|UseVariable|RunOnStart|command`, or `Name|Type|UseVariable|RunOnStart|OpenLogOnRun|command`; omitted booleans default to `false` (since 2.3.1; previously `true` except `OpenLogOnRun`). `--content` commands can set `--open-log-on-run`. `step add` rejects duplicate step names within the same service/template. When `FindStep` by name matches multiple steps, it returns null and the caller reports an ambiguity error requesting a GUID. `step list --json` and `status --json` expose both persisted `Order` and display-oriented metadata, including `OpenLogOnRun`. `start all` has been removed and should not be reintroduced unless the user explicitly asks.
