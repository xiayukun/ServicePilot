...
- Fold collapse state is remembered per group header in `LogWindow._foldStateByHeader` (keyed by the header `LogEntry`, so it survives section destroy/recreate during incremental rebuilds and tab switches). `RebuildFoldings` captures each live section's `IsFolded` into this dict BEFORE `UpdateFoldings`, verifies that every resulting section exactly matches the newly calculated start/end range, and falls back to a full section rebuild only when that postcondition fails; it then reapplies the remembered intent (new headers default to folded). The summary/expand command reconciles foldings before toggling so a stale trailing group cannot remain outside the command's scope. Do not revert to a "fold once" flag: an actively-streaming group's section can be recreated when new child lines arrive, which previously popped a manually-collapsed group back open (fixed in 4.0.2). Only clear this dict when logs are cleared, not on tab switch.
- The log toolbar summary/expand button has no independent toggle state. Its label, tooltip, enabled state, and click action are derived from the current `FoldingManager.AllFoldings`: any expanded section means `摘要` / fold all; all sections folded means `展开` / expand all. Individual fold-margin clicks and search-triggered expansion must immediately capture the header-keyed intent and refresh the button; suppress that event capture while `RebuildFoldings` is applying remembered intent.
