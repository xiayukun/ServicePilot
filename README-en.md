...
![ServicePilot](Assets/servicepilot-hero.png)
...
| --- | --- |
| ![ServicePilot tray context menu](Assets/screenshots/tray-menu-zh.png) | ![Service editor and script actions](Assets/screenshots/service-editor-zh.png) |
...
- **Log merge, folding, and notifications**: actions can define a custom C# merge script that folds multiple lines into one colored summary (VSCode-style expand/collapse), auto-expands search hits, and provides a right-side color overview; scripts can also call `Notify("message")` at live milestones such as startup completion, with frontend startup notifying only once even if HMR reaches `100%` again.
- **Instant tray service filtering**: type a service name immediately after opening the tray menu; the search box and management footer remain pinned while the service list is filtered.
