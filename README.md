...
只要命令行能做到的事，通常都可以包装成 ServicePilot 步骤：切换 API 地址、拉取分支、安装依赖、打开 IDE、启动调试服务。ServicePilot 内置了一个通用模板“默认开发动作模板”，包含 Git 分支/Tag 操作、npm 依赖/构建和常用工具打开入口；也推荐让 AI 先读取 `ai-help`、`doctor --json`、`status --json` 后，直接生成适合当前项目的服务和模板。
...
```text
你可以使用 ServicePilot 管理我的 Windows 本地开发服务。请先运行 ServicePilot.exe ai-help、doctor --json、list --json、status all --json，基于 JSON 结果了解现有服务、步骤、变量和模板。需要新增服务或模板时，优先复用内置的“默认开发动作模板”，也可以直接生成适合当前项目的多步骤服务；删除或覆盖配置前先说明目标名称。
```
