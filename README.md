...
日志窗口（动作日志标签可关闭、支持合并折叠和右侧颜色概览）：

![动作日志标签（可关闭）与高频构建日志折叠](Assets/screenshots/log-window-action-tabs-zh.png)
...
- **可靠停止与日志收尾**：使用 Windows Job Object 清理进程组，减少 Vite/npm 子进程残留占端口；进程完成或停止时会排空 stdout、stderr 和已排队日志，排空超时会明确报告失败。
