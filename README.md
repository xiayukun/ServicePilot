...
**把这段直接丢给 AI：它会先查状态，再帮你生成服务、模板、动作和变量。**

**直接复制给 AI：**

```text
你可以使用 ServicePilot 管理我的 Windows 本地开发服务。请先运行 ServicePilot.exe ai-help、doctor --json、list --json、status all --json，基于 JSON 结果了解现有服务、动作、变量和模板。需要新增服务或模板时，优先复用内置的“默认开发动作模板”，也可以直接生成适合当前项目的多动作服务；删除或覆盖配置前先说明目标名称。
```
...
| 托盘右键菜单 | 编辑服务和脚本动作 |
| --- | --- |
| ![托盘右键菜单](Assets/screenshots/tray-menu-zh.png) | ![编辑服务和脚本动作](Assets/screenshots/service-editor-zh.png) |
