...
- 更新日志合并脚本编译组件 Microsoft.CodeAnalysis.CSharp.Scripting 至 5.9 系列，保留 .NET 8 和现有脚本接口。已验证单文件程序的进度摘要、跨行状态和每次启动仅通知一次。
- 更新 GitHub 构建组件：checkout v7、setup-dotnet v6、upload-artifact v7；保留 Windows x64 自包含单文件产物和隔离 CLI 检查。
- 不修改用户的服务、模板、数据库或运行配置。

## 4.2.1 - 2026-09-02

- **展开/摘要模式会持续作用于新日志组**: 在日志窗口点击“展开”后，后续产生的新合并组也保持展开；点击“摘要”后，新组继续折叠。单独展开某一组或搜索自动展开仍只影响该组。
- **搜索长日志时自动横向定位**: 上一个/下一个搜索命中现在会同时滚动纵轴和横轴，目标位于长行右侧时无需再手动拖动底部滚动条。

## 4.2.0 - 2026-08-25

- **合并函数可发送系统通知**: 合并函数新增 `Notify("通知文字")`，可在识别到启动完成等实时日志时主动发送 Windows 通知；日志窗口关闭时仍然有效，历史日志回放和 `merge-script test` 不会真的弹窗。同一动作五秒内相同文字会自动去重；前端启动完成通知按每次启动动作只提醒一次，热更新再次到 `100%` 不重复提醒。
- **托盘菜单即时筛选服务**: 右键打开托盘菜单后，搜索框会自动获得焦点，可直接输入服务名过滤长列表；筛选框固定在顶部，底部管理操作始终可用，按向下键可进入第一个匹配服务。
- **模板交换保留日志合并脚本**: 导出和重新导入服务模板时会完整保留动作的 `LogMergeScript`，日志折叠、进度摘要和启动完成通知不会因模板分享而丢失。
- **图标透明边缘修复**: 改用带 Alpha 的透明源图，保留完整正方形画布并将四角设为透明；窗口标题栏与任务栏统一直接加载透明 PNG，避免 WPF 二次缩放 ICO 帧产生残留亮边，同时保持图标主体大小。
- **修复日志窗口 UI 线程异常**: 关闭 AvalonEdit 输入时隐藏鼠标指针的可选功能，避免单文件发布环境在鼠标移入日志/脚本编辑器时调用 WinForms 光标 API 导致 `System.Windows.Forms` 找不到的弹窗异常。
- **修复持续输出时末尾日志游离在折叠组外**: 每次增量折叠更新后都会核对 AvalonEdit 中实际折叠区的数量和起止范围；一旦发现与当前日志分组不一致，会立即重建折叠区并恢复用户原有的展开/折叠状态。“摘要/展开”在切换前也会先执行同步，避免末尾几行不受按钮控制。

## 4.1.0 - 2026-07-29

- **动作日志标签可关闭并可靠清空**: 动作日志标签现在按稳定动作标识分组，同名动作不会串到同一标签。可单独关闭某个动作标签并清除其已存日志；工具栏“清空”会清除当前服务的全部日志，关闭或清空都不会停止正在运行的动作。关闭标签的 × 按钮改为透明背景，避免出现突兀的底色。
- **日志滚动与折叠交互更稳定**: 自动滚动会在日志内容和折叠布局实际完成后再定位到底部，高频输出时不再容易停在旧末尾。摘要/展开按钮与每个折叠组的实际展开状态保持同步，持续输出、搜索和切换标签后仍可正确操作。
- **进程停止与尾部日志可靠性**: 进程结束或停止时会等待标准输出、标准错误和已排队日志完成投递，避免短命令的尾部日志丢失；停止期间遇到排空超时会明确报告失败，不再误显示为已成功停止。
